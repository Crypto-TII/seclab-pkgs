...
c28x import path
----------------
This script imports the ``c28x`` package (decoder/types/util) from
``tms320c28x-re``. When packaged, it is a declared dependency of the interpreter
f28335-dump runs this with, so it is simply importable and no search is needed.
...
def _load_c28x() -> None:
    """Import the c28x decoder on first use."""
    global Decoder, BranchType
...
class Classifier:
    ...
    def recover_gaps(
        self, max_rounds: int = 8, min_gap: int = 8, min_calls: int = 2
    ) -> None:
        ...
        for rnd in range(max_rounds):
            ...
            self._log(
                f"  gap-recovery round {rnd + 1}: +{len(new_seeds)} seeds, "
                f"+{gained} code words"
            )
            if gained == 0:
                break
            ...
            while w < end:
                if w in self.covered:
                    ...
                    while w < end and w in self.covered:
                        w += 1
                    out.append(
                        {"start_word": start, "end_word": w, "section": f".{r.name}"}
                    )
                else:
                    w += 1
        return out
    ...
    def data_ranges(
        self,
        padding: set[int] | None = None,
        known_funcs: set[int] | None = None,
        min_calls_code: int = 2,
    ) -> list[dict]:
            ...
            while w < end:
                if (w not in self.covered) and (w not in padding):
                    ...
                    if w - start < self.min_data_words:
                        continue
                    if (
                        len(self.known_call_sites(start, w, known_funcs))
                        >= min_calls_code
                    ):
    ...
    def run(self) -> dict:
        ...
        self._log(
            f"  call targets={len(call_targets)} branch targets={len(branch_targets)}"
        ...
        self._log(
            f"  prologues={len(prologues)} pointer-table entries={len(ptr_table)}"
        )
        ...
        self._log(
            f"classify_f28335: recursive descent from {len(descent_seeds)} seeds ..."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--out",
        help="manifest output path (default: <dump-dir>/dis/dumped.analysis.json)",
    )
    ap.add_argument(
        "--min-data-words",
        type=int,
        default=DEFAULT_MIN_DATA_WORDS,
        help=f"min uncovered span to call data (default {DEFAULT_MIN_DATA_WORDS})",
    )
    ap.add_argument(
        "--min-pad-words",
        type=int,
        default=DEFAULT_MIN_PAD_WORDS,
    ...
    clf = Classifier(
        regions=regions,
        min_data_words=args.min_data_words,
        min_pad_words=args.min_pad_words,
        quiet=args.quiet,
    )
    manifest = clf.run()
...
def classify(dump_dir, out_path) -> None:
    """Library entry point. Raises RuntimeError if the manifest was not written."""
    rc = main([str(dump_dir), "--out", str(out_path)])
    if rc != 0:
        raise RuntimeError(f"classifier exited {rc}")
