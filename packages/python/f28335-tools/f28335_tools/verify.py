...
def main(argv: list[str] | None = None) -> int:
    ...
    for bin_name, sec_name in REGION_SECTIONS:
        bin_path = (
            resolve_flash(dump_dir) if bin_name == "flash.bin" else dump_dir / bin_name
        )
        ...
        if got == want:
            ok += 1
            log(
                f"  {bin_name:18s} == {sec_name:14s} OK  ({len(want)} bytes, "
            ...
            msg = (
                f"{sec_name}: MISMATCH (bin={len(want)}B coff={len(got)}B, "
...
def verify_roundtrip(dump_dir) -> int:
    """Library entry point: 0 on byte-identical, non-zero otherwise."""
    return main([str(dump_dir)])
