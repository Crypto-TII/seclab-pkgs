...
in
# groups.nix repeats these names for the re-environment module, which declares
# one option per group and has no pkgs to ask.
assert lib.assertMsg (lib.attrNames packagesByGroup == import ./groups.nix) ''
  toolsets/groups.nix and toolsets/default.nix disagree:
    groups.nix:  ${lib.concatStringsSep " " (import ./groups.nix)}
...
'';
{
  ...
  allPackages = lib.concatLists (
    lib.mapAttrsToList (
      group: packages: lib.optionals (enabled.${group} or true) packages
    ) packagesByGroup
  );
}
